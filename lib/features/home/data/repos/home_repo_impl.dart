...
class HomeRepoImpl extends HomeRepo {
  ...
  @override
  Future<Either<Failure, List<BookEntity>>> fetchSimilarBooks({required String bookId}) async {
    try {
      List<BookEntity> books = await homeRemoteDataSource.fetchSimilarBooks(bookId: bookId);
      return right(books);
    } catch (e) {
      if (e is DioException) {
        return left(ServerFailure.fromDioException(e));
  ...
  Future<Either<Failure, List<BookEntity>>> searchBooks({required String query}) async {
    try {
      List<BookEntity> books = await homeRemoteDataSource.searchBooks(query: query);
      return right(books);
    } catch (e) {
      if (e is DioException) {
        return left(ServerFailure.fromDioException(e));
